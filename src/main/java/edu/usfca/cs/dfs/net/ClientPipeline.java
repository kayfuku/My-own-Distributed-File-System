...
public class ClientPipeline extends ChannelInitializer<SocketChannel> {
	...
	private SimpleChannelInboundHandler<StorageMessages.StorageMessageWrapper> clientHandler;


	public ClientPipeline(Client client, String destination) {
		
		if (destination.equals("controller")) {
			clientHandler = new ClientToControllerHandler(client);

		} else if (destination.equals("storageNode")) {
			clientHandler = new ClientToStorageNodeHandler(client);

		}
		
		
//		clientHandler = new ClientHandler(client, storageNode);
